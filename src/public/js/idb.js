...
const request = indexedDB.open('abrar-pwa-budget-tracker', 1);
...
request.onupgradeneeded = event => {
    ...
    if (!db.objectStoreNames.contains('abrar-pwa-budget-tracker')) {
        db.createObjectStore('abrar-pwa-budget-tracker', {
            autoIncrement: true
        });
    }
};
...
function checkDatabase() {
    ...
    let transaction = db.transaction(['abrar-pwa-budget-tracker'], "readwrite");

    let store = transaction.objectStore('abrar-pwa-budget-tracker');
